...
## 2025-04-01 - Client-side routing and programmatic focus outlines
**Learning:** Using standard HTML `<a>` tags for internal navigation in Next.js causes full page reloads, destroying client-side state and degrading perceived performance. Furthermore, when implementing skip-to-content links, the target element (like `<main>`) must have `tabIndex="-1"` to receive programmatic focus, but doing so without styling often causes browsers to draw a distracting default focus ring around the entire content area when the link is activated.
**Action:** Always use Next.js `<Link>` components for internal navigation to preserve client-side routing. When creating target elements for skip links, ensure they have `id`, `tabIndex="-1"`, and explicitly apply `outline-none` so the focus shift is accessible but visually invisible.
