...
## 2025-05-08 - Disabling buttons for empty inputs and keyboard submissions
**Learning:** In Next.js client applications, input forms that submit directly to an API route often lack robust visual validation prior to submission. Users may accidentally try to submit empty inputs, resulting in confusing behavior or failed network requests. Specifically, if a submit button is only disabled while `loading`, users can still click it when an associated input is empty. Keyboard users expect forms (even formless inputs) to submit via the Enter key; using `onKeyPress` (which is deprecated in React) or missing key bindings causes frustration.
**Action:** When creating formless text inputs paired with an action button, always add an `onKeyDown` listener (checking `e.key === 'Enter'`) that validates input before triggering the submission function. Explicitly bind the action button's `disabled` state to check for emptiness (e.g., `disabled={loading || !inputValue.trim()}`) and provide a helpful tooltip (`title`) explaining the disabled state to the user. Additionally, ensure the disabled state visually updates CSS properties properly so it appears un-clickable.
