...
export async function resolveTraktId(anilistId, { skipDbCheck = false } = {}) {
  ...
  // otaku mapping (primary)
  console.log(`[ID Resolution] Checking Otaku DB for AniList ${anilistId}...`);
  const otaku = await getOtakuMapping(anilistId);
  if (otaku && otaku.traktId) {
    console.log(`[ID Resolution] Found Trakt ID in Otaku DB: ${otaku.traktId}`);
    await db.saveMapping(anilistId, { ...otaku, traktId: otaku.traktId, type: 'auto' });
    return otaku.traktId;
  }

  // fribbs mapping (secondary)
  console.log(`[ID Resolution] Checking Fribbs for AniList ${anilistId}...`);
  const fribbs = await getFribbsMapping(anilistId);
  if (!fribbs && !otaku) {
    console.warn(`[ID Resolution] No Otaku/Fribbs entry for AniList ${anilistId}`);
    return null;
  }

  let ids = { ...(otaku || {}), ...(fribbs || {}) };
  console.log(`[ID Resolution] IDs from Otaku/Fribbs:`, ids);
