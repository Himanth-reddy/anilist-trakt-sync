...
import { getTraktToken, refreshTraktToken } from './trakt-auth.js';
...
export async function postToTrakt(translatedEpisodes) {
    ...
    const makeRequest = async (token) => {
        return fetch("https://api.trakt.tv/sync/history", {
            ...
            headers: {
                ...
                'trakt-api-version': '2', 'User-Agent': 'Anilist-Trakt-Sync/1.0'
            },
    ...
    try {
        ...
        if (response.status === 401) {
            console.warn('[Trakt] 401 Unauthorized. Attempting to refresh token...');
            try {
