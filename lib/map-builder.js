...
export function createBreakpointMap(seasons) {
    ...
    for (const season of sortedSeasons) {
        ...
        if (hasAbsoluteNumbers && startingEpisode !== Infinity) {
            ...
            // If episode_count suggests a larger season than what we've seen in absolute numbers
            // (e.g. partial airing season), respect episode_count for the next start.
            const impliedEndByCount = startingEpisode + (season.episode_count || season.episodes?.length || 0);
            const impliedEndByMaxAbs = maxAbsInSeason > 0 ? maxAbsInSeason + 1 : nextProjectedStart;

            nextProjectedStart = Math.max(impliedEndByCount, impliedEndByMaxAbs);
        } else {
