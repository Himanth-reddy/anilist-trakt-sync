                ...
                <div className="mb-2">
                    ...
                    <div className="flex gap-3">
                        <input
                            ...
                            onKeyDown={(e) => e.key === 'Enter' && manualId.trim() && !manualLoading && syncShow()}
                            aria-describedby="manualId-hint"
                        />
                        <button
                            onClick={syncShow}
                            disabled={manualLoading || !manualId.trim()}
                            title={!manualId.trim() ? 'Enter an AniList ID to sync' : undefined}
            ...
            <div className="bg-[#111] border border-[#333] p-6 rounded-lg-none mb-6">
                ...
                <div className="flex gap-3 mb-2">
                    <input
                        ...
                        className="flex-1 px-4 py-2 bg-transparent rounded-lg border border-gray-600 focus:border-red-500 focus:outline-none"
                        onKeyDown={(e) => e.key === 'Enter' && authCode.trim() && !authLoading && exchangeTraktCode()}
                    />
                    <button
                        onClick={exchangeTraktCode}
                        disabled={authLoading || !authCode.trim()}
                        title={!authCode.trim() ? 'Enter a Trakt auth code to save' : undefined}
                        aria-busy={authLoading}
